...
async fn test_get_specific_move_effect() {
    ...
    let effect = test_entity.effects.unwrap_or_default();
    let short_effect = test_entity.short_effects.unwrap_or_default();
    ...
    assert!(test_entity.move_ids.contains(&120));
    assert!(test_entity.move_ids.contains(&153));
    assert_eq!(test_entity.move_ids.len(), 2);
